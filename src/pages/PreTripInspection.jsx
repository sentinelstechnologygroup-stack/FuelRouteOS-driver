...
import {
  ShieldCheck, CheckCircle2, AlertTriangle, Camera, ChevronDown, ChevronUp
} from "lucide-react";
import { useDriver } from "@/lib/driverContext";
import { preTripChecklist, truck as truckData, trailer as trailerData } from "@/lib/mockData";
...
export default function PreTripInspection() {
  ...
  const [odometer, setOdometer]       = useState(String(truckData.odometerMi));
  ...
  const categories = [...new Set(preTripChecklist.map(i => i.category))];
  const allChecked = preTripChecklist.every(i => checked[i.id]);
  ...
  const handleSubmit = () => {
    ...
    addSyncItem({ type: "pre_trip", label: "Pre-Trip Inspection", jobId: null, status: "pending" });
    setSubmitted(true);
  };
  ...
  if (submitted) {
    return (
      <div className="bg-background min-h-screen">
        <PageHeader title="Pre-Trip Inspection" backTo="/profile" />
        <div className="px-4 py-8 flex flex-col items-center gap-6 text-center">
          ...
          <div>
            <p className="text-xl font-black">Inspection Complete</p>
            <p className="text-muted-foreground text-sm mt-1">
              Pre-trip saved. Sync pending when online.
            </p>
          </div>
          <div className="w-full bg-card border border-border rounded-2xl p-4 text-left space-y-2">
            <Row label="Truck" value={`#${truckData.number} — ${truckData.year} ${truckData.make} ${truckData.model}`} />
            <Row label="Trailer" value={`#${trailerData.number}`} />
            <Row label="Odometer" value={`${parseInt(odometer).toLocaleString()} mi`} />
            <Row label="Fuel Level" value={fuelLevel} />
            <Row label="Checks Passed" value={`${checkedCount} / ${preTripChecklist.length}`} />
          </div>
          <ActionButton onClick={() => navigate("/today-shift")} variant="primary">
  ...
  return (
    <div className="bg-background min-h-screen pb-8">
      <PageHeader title="Pre-Trip Inspection" subtitle="Required before starting route" backTo="/profile" />
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 h-2 bg-secondary rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${Math.round((checkedCount / preTripChecklist.length) * 100)}%` }}
            />
          </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-4 space-y-3">
          <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold">Vehicle Readings</p>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-muted-foreground">Odometer (mi)</label>
              <input
                type="number"
                value={odometer}
        ...
        {categories.map(cat => {
          const items = preTripChecklist.filter(i => i.category === cat);
            ...
            <div key={cat} className="bg-card border border-border rounded-2xl overflow-hidden">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  {catChecked === items.length && <CheckCircle2 className="w-4 h-4 text-success" />}
                </div>
                <div className="flex items-center gap-2">
        ...
        >
          ...
          {photoCount === 0 ? "Add Inspection Photos (optional)" : `${photoCount} photo${photoCount > 1 ? "s" : ""} attached`}
        </button>
        ...
        <textarea
          ...
          rows={3}
          placeholder="Notes or issues found (optional)…"
          value={notes}
        ...
        {submitAttempted && !allChecked && (
          <p className="text-xs text-destructive font-semibold">
            Complete all {preTripChecklist.length} checklist items before submitting.
          </p>
        )}
        ...
        >
          {allChecked ? "Submit Pre-Trip Inspection" : `Complete Checklist (${checkedCount}/${preTripChecklist.length})`}
        </ActionButton>
      </div>
    </div>
  );
